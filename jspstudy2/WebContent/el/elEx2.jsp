...
noparam 파라미터 값 : <%=request.getParameter("noparam") %><br><br>


<h3>JSP 의 EL(표현)을 이용하여 파라미터와 속성값 출력</h3>
pageContext test 속성값 : ${test}<br>
session test 속성값 : ${sessionScope.test}<br>
today 속성값 : ${today}<br>
name 파라미터값 : ${param.name}<br>
tel 변수값 : EL로 표현못함. <%=tel %><br>
tel 속성값 : ${tel}<br>
...
<%--
	...
	1. pageContext 객체에 등록된 속성 이름이 test인 속성의 값을 리턴
	2. 1번에 해당하는 속성이 없는경우
		request 객체에 등록된 속성 중 이름이 test인 속성의 값을 리턴
	3. 2번에 해당하는 속성이 없는 경우
		session 객체에 등록 된 속성중 이름이 test 인 속성의 값을 리턴
	4. 3번에 해당하는 속성이 없는 경우
		application 객체에 등록된 속성 중 이름이 test 인속성의 값을 리턴
	5. 4번에 해당하는 속성이 없는 경우
		null이 아니고 아무것도 출력하지 않음. 오류 발생도 없음
	--직접 영역 담당 객체 지정하여 출력하기
	pageContext 객체의 속성 : {pageScope.test}
	request 객체의 속성 : {requestScope.test}
	session 객체의 속성 : {sessionScope.test}
	application 객체의 속성 : {application.test}
--%>